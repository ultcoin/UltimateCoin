...
// Copyright (c) 2013-2014 Team UltimateCoin
...
uint256 hashGenesisBlock("0xee68a23e418b44f856ac59e86cb77a38e8b746f122cebb3913e00476f0a202ce");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UltimateCoin: starting difficulty is 1 / 2^12
...
{
    // UltimateCoin: IsDust() detection disabled, allows any valid dust to be relayed.
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            // Transactions under 1K are not free (
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY+40) - GetDepthInMainChain());
}
...
static const int64 nTargetTimespan = 8 * 60 * 60; // UltimateCoin: every 8 hours
static const int64 nTargetSpacing = 240; // UltimateCoin: 4 minutes
     ...
     if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
        ...
        for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                ...
                if (LatestBlockTime < BlockReading->GetBlockTime()) {
                        if (BlockReading->nHeight > 10300) LatestBlockTime = BlockReading->GetBlockTime();
                }
...
{
        static const int64        BlocksTargetSpacing                        = 240; // 4 minutes
...
{
    // UltimateCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd5710a00bd80c20cce47749caa03b48ae2940322c4658aa57bc941152f7c2df4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Chairman Of Joint Chiefs: US Ready For Military Response In Ukraine";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394668056;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 314644;

        if (fTestNet)
        {
            block.nTime    = 1394638207;
            block.nNonce   = 2905160;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe6526c9a15f96dc2aee2af47080cd293809f6fd173f640ce57981cf61f7a181e"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UltimateCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UltimateCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UltimateCoinMiner(CWallet *pwallet)
{
    printf("UltimateCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ultimatecoin-miner");
    ...
    try { loop {
        ...
        printf("Running UltimateCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UltimateCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UltimateCoinMiner, pwallet));
}
