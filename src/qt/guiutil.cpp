...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("ultimatecoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("ultimatecoin://"))
    {
        uri.replace(0, 11, "ultimatecoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "UltimateCoin.lnk";
}
...
{
    return GetAutostartDir() / "ultimatecoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("UltimateCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ultimatecoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("UltimateCoin-Qt"));
