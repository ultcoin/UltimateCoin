...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "UltimateCoin " + FormatFullVersion();
...
// Nothing for now until testing is over with
    {"dnsseeds.ultcoin.org", "ns1.ultcoin.org"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. UltimateCoin is probably already running."), addrBind.ToString().c_str());
        else
